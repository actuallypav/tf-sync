...
#store the current directory as the Project Directory
...
if [[ ! -d "$LOCAL_STATE_DIR/.git" ]]; then
    ...
    echo "Updating TFstate repo from GitHub..."
    cd "$LOCAL_STATE_DIR"
...
#find the latest backup directory **from the updated GitHub repo**
LATEST_BACKUP=$(ls -td "$LOCAL_STATE_DIR/$PROJECT_NAME/backups/"*/ 2>/dev/null | head -1)

if [[ -z "$LATEST_BACKUP" ]]; then
    echo "⚠️ No backups found in GitHub for $PROJECT_NAME. Skipping sync."
    exit 0
fi
...
if [[ ! -f "$LATEST_REMOTE_FILE" ]]; then
    echo "⚠️ No terraform.tfstate file found in the latest GitHub backup for $PROJECT_NAME. Skipping sync."
    exit 0
fi
...
echo "🔄 Updating terraform state from GitHub..."
...
echo "✅ TFstate successfully updated for $PROJECT_NAME from latest GitHub backup."
